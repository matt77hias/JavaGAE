...
@NamedQuery(
    ...
    query= "SELECT DISTINCT company.carTypes FROM CarRentalCompany company "
...
public class CarRentalCompany implements Serializable {
    ...
    private static final long serialVersionUID = -6926912979478466982L;
    ...
    public CarRentalCompany(String name, Collection<Car> cars, Set<CarType> carTypes) {
    ...
    public Set<CarType> getAvailableCarTypes(Date start, Date end) {
        ...
        for (Car car : cars) {
            System.out.println("checking car " + car.getId());
            if (car.isAvailable(start, end)) {
                availableCarTypes.add(getCarTypeOfCar(car));
            }
        }
    ...
    public CarType getCarTypeOfCar(Car car)
    	throws IllegalArgumentException, IllegalStateException {
    	if (!cars.contains(car))
    		throw new IllegalArgumentException();
    	for (CarType carType : carTypes) {
    		if (carType.getId().equals(car.getCarTypeId()))
    			return carType;
    	}
    	throw new IllegalStateException("could not find carType " + car.getCarTypeId());
    }
    ...
    public Set<Car> getCars(CarType type) {
        ...
        for (Car car : cars) {
            if (getCarTypeOfCar(car).equals(type)) {
                out.add(car);
            }
        }
        return out;
     ...
     public Set<Car> getCars(String type) {
        Set<Car> out = new HashSet<Car>();
        for (Car car : cars) {
            if (type.equals(getCarTypeOfCar(car).getName())) {
                out.add(car);
            }
        }
        return out;
    ...
    private List<Car> getAvailableCars(String carType, Date start, Date end) {
        ...
        for (Car car : cars) {
            if (getCarTypeOfCar(car).getName().equals(carType) && car.isAvailable(start, end)) {
